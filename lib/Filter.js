function Filter(rawFilter) {
    ...
    for(var i = 0; i < stringFilters.length; i++) {
        var value = rawFilter[stringFilters[i]];
        this[stringFilters[i]] = (!value || (Array.isArray(value) && value.length === 0)) ? '' : value;
    }
    ...
    this.test = function (post) {
        ...
        for(var i = 0; i < stringFilters.length; i++) {
            ...
            if (!stringContains(post[filter], this[filter])) {
                console.log(post[filter] + " does not contain " + this[filter]);
                return false;
            }
        }
